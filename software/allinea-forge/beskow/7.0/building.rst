...
As install base directory select::
  ...
  /pdc/vol/allinea-forge/7.0

License needs to be adapted::

  /pdc/vol/allinea-forge/License/License

Then create /pdc/vol/allinea-forge/7.0/system.config.template.
...
Then we create the profiler libraries::
  ...
  $ mkdir -p /pdc/vol/allinea-forge/7.0/lib/INTEL
  $ cd /pdc/vol/allinea-forge/7.0/lib/INTEL
  $ /pdc/vol/allinea-forge/7.0/bin/make-profiler-libraries --platform=cray --without-shmem

  $ module swap PrgEnv-intel PrgEnv-gnu
  $ mkdir -p /pdc/vol/allinea-forge/7.0/lib/GNU
  $ cd /pdc/vol/allinea-forge/7.0/lib/GNU
  $ /pdc/vol/allinea-forge/7.0/bin/make-profiler-libraries --platform=cray --without-shmem

  $ module swap PrgEnv-gnu PrgEnv-cray
  $ mkdir -p /pdc/vol/allinea-forge/7.0/lib/CRAY
  $ cd /pdc/vol/allinea-forge/7.0/lib/CRAY
  $ /pdc/vol/allinea-forge/7.0/bin/make-profiler-libraries --platform=cray --without-shmem

Also created the following helper script and placed it in /pdc/vol/allinea-forge/7.0/bin/copy-allinea-forge-config:
...
Which makes use of pkg-config files like this one (placed in /pdc/vol/allinea-forge/7.0/lib/INTEL):
