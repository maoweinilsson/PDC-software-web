...
In this example, we requested one node for one hour and have chosen to run on full processors per node with *cfx5solve*. Note the above *Benchmark.def* should be replaced by your definition file and *NUM_PART* by the partition count. If the NUM_PART is NOT specified, a partition file (.par) is assumed to be created in the same directory.
